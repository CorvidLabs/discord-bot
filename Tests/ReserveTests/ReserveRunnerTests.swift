...
struct ReserveRunnerTests {
    ...
    func aResumedEpochNamesBothCeilings() async throws {
        ...
        // Typed, because the element type is inferred from the literal and
        // Linux infers `Int` here where macOS agrees with the key path.
        let checked: [UInt64] = [269_231 * 3, 269_231 * 2]
        #expect(record.charges.map(\.checkedWholeUnits) == checked)
