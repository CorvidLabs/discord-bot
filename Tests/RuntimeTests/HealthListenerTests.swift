...
internal struct HealthListenerTests {
    ...
    internal func aSilentClientDoesNotHoldUpTheEndpoint() async throws {
        ...
        // does not hold the endpoint until its own read timeout, which is
        // tens of seconds: any bound well under that catches the regression.
        // One second sat close enough to an ordinary scheduling hiccup on a
        // loaded CI machine that it failed at 1.10s while the listener was
        // behaving correctly, and a test that fails for reasons outside what
        // it names teaches people to rerun rather than read.
        #expect(
            took < .seconds(3),
