...
struct SQLiteFileTests {
    ...
    // Disabled for root, which ignores the permission bits this test relies
    // on: the copy it is supposed to be unable to take succeeds, and the
    // assertion fails for a reason that has nothing to do with the code. CI
    // runs the Linux container as root because the runner's own file commands
    // need it. Skipped rather than weakened, so it still proves what it is
    // named for on a developer machine and in a container run as a user.
    @Test(
        "A copy that cannot be taken stops the migration rather than being skipped (RUN-9)",
        .enabled(if: geteuid() != 0, "permission bits mean nothing to root")
    )
