...
- **Tolerant about spelling, strict about a blob that says two things.** The
  ...
  actually sent. Padded or not is the whole of that tolerance, and the
  reader judges it rather than Foundation: the two implementations disagree
  in opposite directions, so a blob padded into nonsense reached the parser
  on Linux and was refused at the decode on Darwin, and the member read the
  fourth reason or the third depending on which machine answered. An
  ordered list of reasons whose answer depends on the host is not an
  ordered list. It refuses a duplicate key at any depth and any byte after the
