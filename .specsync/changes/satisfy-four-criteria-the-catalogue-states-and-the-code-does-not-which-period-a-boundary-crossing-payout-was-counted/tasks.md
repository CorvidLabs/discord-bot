...
- [x] Re-read the runtime change's definition in its own workspace and confirm
...
- [x] Confirm the second edge with that change as well: stage 3 makes the
...
- [x] Record the ordering with `specsync change depend` once both workspaces
...
- [x] Confirm with the approver which of the two readings of RUN-11 is wanted
...
- [x] Add a test to `Tests/ChainTests/RequestGovernorTests.swift` citing
      RUN-10.a, over a **provider quota refusal** pause on a budget with
      headroom (budget 100, 40 spent), not an exhausted budget.
- [x] Assert the snapshot before and after `unpause` is identical in
      `usedRequests`, `remainingRequests` and `dayStart`.
- [x] Assert `remainingRequests(now:)` still answers the same number after the
      unpause.
- [x] Assert the day still ends where it would have: the sixtieth further
      request is allowed and the sixty-first throws
      `ChainError.requestBudgetSpent`.
- [x] Assert against a store double that the counts written are non-decreasing
      and never drop below what was spent before the unpause.
- [x] Run the five mutations in the table in `plan.md` by hand against the new
      ...
      note.
- [x] Leave the existing test in place and add the RUN-10.a citation to it too.
- [x] Add the criterion id to the `Covered by` line of `REQ-chain-008` in
...
- [x] Add a non-probing read to `ProviderProofProbe` that answers with the
      ...
      untouched.
- [x] Leave `proof(now:)` exactly as it is, including the rule that a failed
      probe drops the stale answer rather than serving it on.
- [x] Add the `RequestBudgetSnapshot` to `ChainHealthReport` as an optional
      member, with a documented reason it is optional: a host with no chain
      configured has no budget to report.
- [x] Extend `jsonBody` with the budget section, appended after the existing
      keys, with the same hand-built escaping discipline and a fixed key order.
- [x] Leave `status` at its two cases (R-HEALTH-4): assert in a test that a
...
- [x] Implement the share as a refilling allowance: the sustained rate is that
...
- [x] Refuse a percentage above one hundred at boot, naming the variable, and
      treat zero as shares off. A burst larger than the day's share clamps to
      it and is reported, not refused.
- [x] Put the arithmetic in a pure value beside the actor, the way `TokenBucket`
      sits beside `RequestRateLimiter`, so every boundary has a test that
      finishes instantly.
- [x] Enforce the share inside `RequestGovernor.spend` and nowhere else: pause
      check first, share second without mutating anything, day third.
- [x] Make the per-caller reservation take the caller with **no default**, on
      `RequestGovernor` and on the read surfaces of `ChainReader`,
      `BatchedChainReader` and `WalletCheckCache`.
- [x] Update every call site in `Sources/` and `Tests/` to say whose work it is.
      A call site that is genuinely system work says so explicitly.
- [x] Add a `ChainError` case for a caller over their share, distinct from
...
- [x] Apply the all-or-nothing rule to `reserveRequests(_:now:)` for a member
      caller as well: the whole count against the remaining share, or nothing.
- [x] Hold one refilling allowance per caller in memory, dropping an entry once
...
- [x] Cap the number of tracked callers with a constant. At the cap, refuse a
...
- [x] Add the share size and the number of callers that reached it today to
      `RequestBudgetSnapshot`. A count, never a list of who (R-SHARE-9).
- [x] Test, citing RUN-11: one member cannot spend the day. A member spending
...
- [x] Test: a refusal on the share does not pause the process, does not change
      the day's counter, and records no notice.
- [x] Test: system work is not metered, so a sweep larger than a member's share
      completes.
- [x] Test: an allowance refills towards the burst and no further, so a member
      is not locked out for the rest of the day and an idle member does not
      accumulate the whole day's share.
- [x] Test: a restart hands a caller at most one fresh burst, while the day's
      own count is restored from the store (R-SHARE-8, RUN-8.b).
- [x] Test: with no daily budget set there is no share, and nothing is refused.
- [x] Test: at the tracked-caller cap, a new caller is refused and one notice is
      recorded, not one per refusal.
- [x] Extend the stage 1 test so that `unpause` also returns none of the
      caller's share, and add that mutation to the table in `plan.md`.
- [x] Add a new requirement to `specs/chain/requirements.md` with the next free
...
- [x] Note in the contract that the ledger does not survive a restart, so the
      gap is written where somebody will read it.

## Stage 4: SPEND-9.c, which period an epoch was counted against

- [x] Add the charge value to `Reserve`: the period key of the limits a run was
...
- [x] Read and write the new table in
...
- [x] Carry the charges through `Sources/Store/InMemoryStore.swift` and the
      `Reserve` in-memory store, and confirm both agree with the file-backed
      one.
- [x] Add conformance behaviours in
...
- [x] Add a test citing SPEND-9.c that a finished epoch names the period it was
      charged to, using the injected clock so any boundary is crossed
      deterministically.
- [x] Add a test citing SPEND-9.c and SPEND-9.a that an epoch cut short part way
      through and resumed in a later period names both, in run order.
- [x] Add a test that an epoch run under a payer stating no limits records no
      charge at all, and that nothing is invented from the cadence key
      (R-PERIOD-4).
- [x] Update `specs/reserve/reserve.spec.md`: the `ReserveEpochRecord` and
...
- [x] Update `specs/store/store.spec.md`: the new table, the conformance
      behaviour list, the migration log entry for version 3, and the Change
      Log.
- [x] Update `specs/reserve/testing.md` and `specs/store/testing.md` with the
      new tests.

## Stage 5: closing the change

- [x] Write the semantic deltas into the workspace's `deltas/` directory, one
      per affected canonical spec: `reserve`, `chain`, `store`.
- [x] Confirm `affected_paths` still covers what the change touches. It now
...
- [x] Check every `Covered by` line touched names a real test that really covers
      it, by running that test alone and watching it fail against a deliberate
      break.
- [x] `specsync change check <id>` clean.
- [x] `specsync check --strict` clean, with coverage unchanged at full.
- [x] `fledge lanes run verify` green, which is `swift build` then `swift test`,
      and is the same command the Trust gate runs.
- [x] Confirm the suite still runs with no network, no credentials and no
      database anybody installed, including the new tests.
- [x] Confirm no new public symbol lacks a doc comment and no new declaration
      lacks an access modifier.
- [x] Read the whole diff once for the house rules: no force unwrap, no `try!`,
      no `as!`, no single-letter generics, no `Double` near an amount.
- [x] Record in `context.md` anything decided differently from `plan.md`, with
